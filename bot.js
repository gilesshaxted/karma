...
client.on('interactionCreate', async interaction => {
    // Crucial: Acknowledge interaction immediately to prevent "Application did not respond"
    let acknowledged = false;
    try {
        if (interaction.isCommand() || interaction.isButton()) {
            await interaction.deferReply({ ephemeral: true }); // Always defer ephemeral for commands/buttons
            acknowledged = true;
        }
    } catch (deferError) {
        if (deferError.code === 10062) { // Unknown interaction (already expired or acknowledged)
        ...
        console.error(`Error deferring interaction ${interaction.id}:`, deferError);
        // If defer fails for other reasons, we might still try to reply later if not acknowledged
    }

    // Now, check bot readiness
    if (!client.db || !client.appId || !client.googleApiKey) {
    ...
    try {
        if (interaction.isCommand()) {
            ...
            await command.execute(interaction, {
                ...
                addKarmaPoints: karmaSystem.addKarmaPoints, // New
                subtractKarmaPoints: karmaSystem.subtractKarmaPoints, // New
                setKarmaPoints: karmaSystem.setKarmaPoints, // New
                client
            });
        ...
        if (acknowledged) { // Only try to edit if we successfully deferred earlier
            ...
            console.error('Interaction not acknowledged and unexpected error occurred. Cannot respond.');
        }
    }
});
